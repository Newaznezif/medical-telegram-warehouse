...
class YOLODetector:
    ...
    def __init__(self, model_path: Optional[str] = None, device: str = 'cuda'):
        ...
        self.image_base_path = Path(settings.IMAGE_BASE_PATH) if hasattr(settings, 'IMAGE_BASE_PATH') else Path("data/raw/images")
        self.output_dir = Path(settings.DETECTION_OUTPUT_PATH) if hasattr(settings, 'DETECTION_OUTPUT_PATH') else Path("data/processed/detections")
    ...
    def load_to_postgres(self, df: pd.DataFrame, table_name: str = "raw_image_detections"):
        """
        Load detection results to PostgreSQL database
        
        Args:
            df: DataFrame containing detection results
            table_name: Name of the table to load data into
        """
        if df.empty:
            logger.warning("No detections to load to database")
            return
        
        try:
            from scripts.load_json_to_postgres import DatabaseConnection
            
            logger.info(f"Loading {len(df)} detections to PostgreSQL table: {table_name}")
            ...
            for _, row in df.iterrows():
                record = (
                    ...
                    row['original_width'],
                    row['original_height'],
                    row['processed_at'].isoformat() if isinstance(row['processed_at'], datetime) else row['processed_at']
                )
            ...
            CREATE TABLE IF NOT EXISTS {table_name} (
                ...
                x_center FLOAT NOT NULL CHECK (x_center >= 0 AND x_center <= 1),
                y_center FLOAT NOT NULL CHECK (y_center >= 0 AND y_center <= 1),
                box_width FLOAT NOT NULL CHECK (box_width >= 0 AND box_width <= 1),
                box_height FLOAT NOT NULL CHECK (box_height >= 0 AND box_height <= 1),
                original_width INTEGER NOT NULL,
                original_height INTEGER NOT NULL,
                processed_at TIMESTAMP NOT NULL,
                created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
                UNIQUE(image_path, detected_class, confidence, x_center, y_center)
            );
            
            CREATE INDEX IF NOT EXISTS idx_{table_name}_class ON {table_name}(detected_class);
            CREATE INDEX IF NOT EXISTS idx_{table_name}_channel ON {table_name}(channel_name);
            CREATE INDEX IF NOT EXISTS idx_{table_name}_confidence ON {table_name}(confidence DESC);
            """
            
            # Insert data
            insert_sql = f"""
            INSERT INTO {table_name} 
            (image_path, image_name, channel_name, date_str, detected_class, 
             confidence, x_center, y_center, box_width, box_height,
             original_width, original_height, processed_at)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
            ON CONFLICT (image_path, detected_class, confidence, x_center, y_center) 
            DO NOTHING
            """
            ...
            with DatabaseConnection() as conn:
                ...
                logger.info(f"Successfully loaded {len(records)} detections to {table_name}")
                logger.info(f"Total records in table: {total_count}")
                
        except ImportError:
            logger.error("Database connection module not found. Install required dependencies.")
            raise
        except Exception as e:
...
def parse_arguments():
    ...
    parser.add_argument('--output-dir', type=str, default=None,
                       help='Output directory for results (default: data/processed/detections)')
    parser.add_argument('--load-to-db', action='store_true',
                       help='Load results to PostgreSQL database')
    parser.add_argument('--table-name', type=str, default='raw_image_detections',
                       help='Table name for database insertion (default: raw_image_detections)')
...
def main():
    ...
    try:
        ...
        if not detections_df.empty:
            ...
            if args.load_to_db:
                logger.info(f"Loading results to database table: {args.table_name}")
                detector.load_to_postgres(detections_df, table_name=args.table_name)
